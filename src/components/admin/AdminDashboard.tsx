...
export const AdminDashboard: React.FC = () => {
  ...
  const filteredProducts = products.filter((prod) => {
    ...
    if (q) {
      const searchable = [prod.nameBn, prod.nameEn, prod.slug || '', category?.nameBn || '', category?.nameEn || '', category?.slug || ''].join(' ').toLowerCase();
      if (!searchable.includes(q)) return false;
    }
    if (productStatusFilter === 'active' && !prod.isActive) return false;
    if (productStatusFilter === 'inactive' && prod.isActive) return false;
    if (productCategoryFilter !== 'all' && prod.categoryId !== productCategoryFilter) return false;
    return true;
  });
      ...
      {activeTab === 'inventory' && (
        <div className="space-y-4">
          ...
          <div className="bg-white p-3 sm:p-4 rounded-2xl border border-stone-200">
            <div className="flex flex-col sm:flex-row gap-2.5">
              <div className="relative flex-1">
                <input type="search" value={productSearchQuery} onChange={(e) => setProductSearchQuery(e.target.value)} placeholder="পণ্যের নাম, ইংরেজি নাম, স্লাগ বা ক্যাটাগরি খুঁজুন..." className="w-full bg-stone-50 text-stone-900 text-xs sm:text-sm pl-9 pr-9 py-2.5 rounded-xl border border-stone-300 focus:outline-hidden focus:border-emerald-600" aria-label="পণ্য খুঁজুন" />
                <Search className="w-4 h-4 text-stone-400 absolute left-3 top-1/2 -translate-y-1/2" />
                {productSearchQuery && <button type="button" onClick={() => setProductSearchQuery('')} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-stone-400 hover:text-stone-700 text-xs p-1" aria-label="সার্চ পরিষ্কার করুন">✕</button>}
              </div>
              <select value={productCategoryFilter} onChange={(e) => setProductCategoryFilter(e.target.value)} className="w-full sm:w-auto bg-stone-50 text-stone-800 text-xs px-3 py-2.5 rounded-xl border border-stone-300 focus:outline-hidden" aria-label="পণ্য ক্যাটাগরি ফিল্টার">
                <option value="all">সব ক্যাটাগরি</option>
                {categories.map((category) => <option key={category.id} value={category.id}>{category.nameBn}</option>)}
              </select>
              <select value={productStatusFilter} onChange={(e) => setProductStatusFilter(e.target.value as typeof productStatusFilter)} className="w-full sm:w-auto bg-stone-50 text-stone-800 text-xs px-3 py-2.5 rounded-xl border border-stone-300 focus:outline-hidden" aria-label="পণ্য স্ট্যাটাস ফিল্টার">
                <option value="all">সব স্ট্যাটাস</option><option value="active">সক্রিয়</option><option value="inactive">নিষ্ক্রিয়</option>
              </select>
            </div>
            {(productSearchQuery || productCategoryFilter !== 'all' || productStatusFilter !== 'all') && (
              <div className="flex items-center justify-between gap-2 mt-2.5 pt-2.5 border-t border-stone-100">
                <span className="text-[11px] text-stone-500">{filteredProducts.length}টি পণ্য পাওয়া গেছে</span>
                <button type="button" onClick={() => { setProductSearchQuery(''); setProductCategoryFilter('all'); setProductStatusFilter('all'); }} className="text-[11px] font-bold text-emerald-700 hover:text-emerald-800">ফিল্টার পরিষ্কার করুন</button>
              </div>
            )}
          </div>
          ...
          <div className="bg-white rounded-2xl border border-stone-200 overflow-hidden shadow-2xs">
            <div className="hidden md:block overflow-x-auto">
              <table className="w-full text-left text-xs sm:text-sm">
                <thead className="bg-stone-50 border-b border-stone-200 text-stone-600"><tr><th className="p-3">ছবি ও নাম</th><th className="p-3">ক্যাটাগরি</th><th className="p-3">বিক্রয় মূল্য</th><th className="p-3">স্টক</th><th className="p-3">স্ট্যাটাস</th><th className="p-3 text-right">একশন</th></tr></thead>
                <tbody className="divide-y divide-stone-100">
                  {filteredProducts.length > 0 ? filteredProducts.map((prod) => {
                    const cat = categories.find((c) => c.id === prod.categoryId);
                    return (
                      <tr key={prod.id} className="hover:bg-stone-50/70">
                        ...
                        <td className="p-3 text-stone-600">{cat?.nameBn || 'অনির্দিষ্ট'}</td>
                        <td className="p-3 font-bold text-stone-900">{formatPrice(prod.price)}{prod.regularPrice ? <span className="text-[10px] text-stone-400 line-through block font-normal">{formatPrice(prod.regularPrice)}</span> : null}</td>
                        <td className="p-3"><span className={prod.stock <= 3 ? 'font-semibold text-rose-600' : 'font-semibold text-stone-800'}>{prod.stock} {prod.unit || 'টি'}</span></td>
                        <td className="p-3"><span className={'text-[11px] font-bold px-2 py-0.5 rounded-full ' + (prod.isActive ? 'bg-emerald-50 text-emerald-800' : 'bg-stone-100 text-stone-500')}>{prod.isActive ? 'সক্রিয়' : 'নিষ্ক্রিয়'}</span></td>
                        <td className="p-3 text-right space-x-1.5">
                          <button onClick={() => { setEditingProduct(prod); setIsProductModalOpen(true); }} className="p-1.5 text-stone-600 hover:text-emerald-700 hover:bg-stone-100 rounded-lg transition-colors" title="এডিট করুন"><Edit2 className="w-3.5 h-3.5" /></button>
                          <button onClick={() => { if (window.confirm('আপনি কি "' + prod.nameBn + '" পণ্যটি মুছে ফেলতে চান?')) { deleteProduct(prod.id); showToast('পণ্যটি মুছে ফেলা হয়েছে'); } }} className="p-1.5 text-stone-600 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="মুছে ফেলুন"><Trash2 className="w-3.5 h-3.5" /></button>
                        </td>
                      </tr>
                    );
                  }) : <tr><td colSpan={6} className="p-8 text-center text-stone-500">কোনো পণ্য পাওয়া যায়নি।</td></tr>}
              ...
              {filteredProducts.length > 0 ? filteredProducts.map((prod) => {
                ...
                return (
                  <div key={prod.id} className="p-3.5 space-y-3">
                    <div className="flex items-start gap-3">
                      <img src={prod.imageUrl} alt={prod.nameBn} className="w-14 h-14 rounded-xl object-cover bg-stone-100 border border-stone-100 shrink-0" />
                      <div className="min-w-0 flex-1"><div className="font-bold text-stone-900 leading-5">{prod.nameBn}</div><div className="text-[11px] text-stone-400 truncate">{prod.nameEn || '—'}</div><div className="text-[11px] text-stone-500 mt-1 truncate">{cat?.nameBn || 'অনির্দিষ্ট'}</div></div>
                      <div className="text-right shrink-0"><div className="font-black text-stone-900">{formatPrice(prod.price)}</div>{prod.regularPrice ? <div className="text-[10px] text-stone-400 line-through">{formatPrice(prod.regularPrice)}</div> : null}</div>
                    </div>
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-center gap-2 min-w-0"><span className={'text-[11px] font-bold px-2 py-1 rounded-lg ' + (prod.isActive ? 'bg-emerald-50 text-emerald-800' : 'bg-stone-100 text-stone-500')}>{prod.isActive ? 'সক্রিয়' : 'নিষ্ক্রিয়'}</span><span className={prod.stock <= 3 ? 'text-[11px] font-bold text-rose-600' : 'text-[11px] font-semibold text-stone-700'}>স্টক: {prod.stock} {prod.unit || 'টি'}</span></div>
                      <div className="flex items-center gap-1 shrink-0">
                        <button type="button" onClick={() => { setEditingProduct(prod); setIsProductModalOpen(true); }} className="p-2 text-stone-600 hover:text-emerald-700 hover:bg-stone-100 rounded-lg" title="এডিট করুন" aria-label={prod.nameBn + ' এডিট করুন'}><Edit2 className="w-4 h-4" /></button>
                        <button type="button" onClick={() => { if (window.confirm('আপনি কি "' + prod.nameBn + '" পণ্যটি মুছে ফেলতে চান?')) { deleteProduct(prod.id); showToast('পণ্যটি মুছে ফেলা হয়েছে'); } }} className="p-2 text-stone-600 hover:text-rose-600 hover:bg-rose-50 rounded-lg" title="মুছে ফেলুন" aria-label={prod.nameBn + ' মুছে ফেলুন'}><Trash2 className="w-4 h-4" /></button>
                      </div>
                    </div>
                  </div>
                );
          ...
          </div> useState, useEffect } from 'react';
import { useShop } from '../../context/ShopContext';
import { Product, Category, Order, OrderStatus, ShopSettings } from '../../types';
import { formatPrice } from '../../utils/helpers';
import { supabaseSendPasswordResetEmail, supabaseUpdatePassword } from '../../lib/supabase';
